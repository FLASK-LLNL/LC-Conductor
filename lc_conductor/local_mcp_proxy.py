...
from lc_conductor.tooling import MCPToolDefinition, ToolDescriptor, ToolRuntime
...
def build_local_mcp_direct_tools(
    websocket: WebSocket,
    server_tool_map: dict[str, list[MCPToolDefinition]] | None,
) -> list[ToolDescriptor]:
    ...
    for server_url, tool_definitions in server_tool_map.items():
        for tool_definition in tool_definitions:
            ...
            tools.append(
                ToolDescriptor(
                    kind="mcp",
                    identifier=f"{server_url}::{tool_definition.name}",
                    server=server_url,
                    names=[tool_definition.name],
                    description=tool_definition.description
                    or f"Proxy MCP tool `{tool_definition.name}`.",
...
def attach_local_mcp_tools(
    websocket: WebSocket,
    runtime: ToolRuntime,
) -> ToolRuntime:
    if not runtime.local_mcp_tools:
        return runtime

    return ToolRuntime(
        tools=[
            *runtime.tools,
            *build_local_mcp_direct_tools(websocket, runtime.local_mcp_tools),
        ],
    )
